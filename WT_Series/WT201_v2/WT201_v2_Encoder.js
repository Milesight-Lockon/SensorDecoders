...
function milesightDeviceEncoder(payload) {
    ...
    if ("d2d_master_config" in payload) {
        for (var i = 0; i < payload.d2d_master_config.length; i++) {
            ...
            encoded = encoded.concat(setD2DMasterConfig(d2d_config.plan_type, d2d_config.enable, d2d_config.d2d_cmd, d2d_config.uplink_enable, d2d_config.time_enable, d2d_config.time));
        }
    }
    if ("d2d_slave_config" in payload) {
        for (var i = 0; i < payload.d2d_slave_config.length; i++) {
            var d2d_config = payload.d2d_slave_config[i];
            encoded = encoded.concat(setD2DSlaveConfig(d2d_config.id, d2d_config.enable, d2d_config.d2d_cmd, d2d_config.action_type, d2d_config.action));
        }
    }
 ...
 * @param {number} source, values: (0: "disable", 1: "lora", 2: "d2d")
 * @param {number} timeout, unit: minute, range: [5, 60]
 * @example { "outside_temperature_control": { "source": 1, "timeout": 10 } }
 */
function setOutsideTemperatureControl(source, timeout) {
    var outside_temperature_source_values = [0, 1, 2];
    if (outside_temperature_source_values.indexOf(source) === -1) {
        throw new Error("outside_temperature_control.source must be one of " + outside_temperature_source_values.join(", "));
    }
    if (source && typeof timeout !== "number") {
        throw new Error("outside_temperature_control.timeout must be a number");
    }
    if (source && (timeout < 5 || timeout > 60)) {
 ...
 * @param {number} fan_delay_time unit: second, range: [1, 3600]
...
function setFanModeWithDelay(fan_delay_enable, fan_delay_time) {
    ...
    if (fan_delay_enable && (fan_delay_time < 1 || fan_delay_time > 3600)) {
        throw new Error("fan_delay_time must be in range [1, 3600]");
    }
 ...
 * @example { "plan_type": 0 }
 */
function setPlanMode(plan_type) {
    var plan_type_values = [0, 1, 2, 3, 4, 5, 6];
...
function setPlanSchedule(type, id, enable, week_recycle, time) {
    var plan_type_values = [0, 1, 2, 3, 4, 5, 6];
    if (plan_type_values.indexOf(type) === -1) {
        throw new Error("plan_schedule[].type must be one of " + plan_type_values.join(", "));
    }
    if (typeof id !== "number") {
...
function setPlanConfig(type, temperature_control_mode, fan_mode, temperature_target, temperature_error, temperature_unit) {
    var plan_type_values = [0, 1, 2, 3, 4, 5, 6];
    var plan_temperature_control_mode_values = [0, 1, 2, 3];
    var plan_fan_mode_values = [0, 1, 2];
    if (plan_type_values.indexOf(type) === -1) {
 ...
 * @param {number} plan_type values: (0: wake, 1: away, 2: home, 3: sleep, 4: occupied, 5: vacant, 6: eco)
 * @param {number} enable values: (0: disable, 1: enable)
 ...
 */
function setD2DMasterConfig(plan_type, enable, d2d_cmd, uplink_enable, time_enable, time) {
    var plan_type_values = [0, 1, 2, 3, 4, 5, 6];
    if (plan_type_values.indexOf(plan_type) === -1) {
        throw new Error("d2d_master_config.plan_type must be one of " + plan_type_values.join(", "));
    }
    var enable_values = [0, 1];
    if (enable_values.indexOf(enable) === -1) {
 ...
 * @param {number} action values: action_type=0 (0: off, 1: on), action_type=1 (0: wake, 1: away, 2: home, 3: sleep, 4: occupied, 5: vacant, 6: eco)
...
function setD2DSlaveConfig(id, enable, d2d_cmd, action_type, action) {
    ...
    if (action_type === 1) {
        var action_values = [0, 1, 2, 3, 4, 5, 6];
        if (action_values.indexOf(action) === -1) {
            throw new Error("d2d_slave_config.action must be one of " + action_values.join(", "));
        }
    }

...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // TEMPERATURE CONTROL
            ...
            // temperature_control_mode(0..1) + temperature_control_status(4..7)
        ...
        // PLAN
            ...
            var plan_schedule = {};
            plan_schedule.type = readPlanType(bytes[i]);
            plan_schedule.id = bytes[i + 1] + 1;
            plan_schedule.plan_enable = readPlanEnable(bytes[i + 2]);
            plan_schedule.week_recycle = readWeekRecycleSettings(bytes[i + 3]);
            plan_schedule.time = readUInt16LE(bytes.slice(i + 4, i + 6));
            i += 6;
        ...
        // PLAN SETTINGS
            ...
            var plan_config = {};
            plan_config.type = readPlanType(bytes[i]);
            plan_config.temperature_control_mode = readTemperatureCtlMode(bytes[i + 1]);
            plan_config.fan_mode = readFanMode(bytes[i + 2]);
            plan_config.temperature_target = readInt16LE(bytes.slice(i + 3, i + 5)) / 10;
            plan_config.temperature_error = readUInt8(bytes[i + 5]) / 10;
            i += 6;
        ...
        // TARGET TEMPERATURE OVER RANGE
            ...
            decoded.temperature_target_alarm = {};
            decoded.temperature_target_alarm.temperature_target = readInt16LE(bytes.slice(i, i + 2)) / 10;
            decoded.temperature_target_alarm.temperature_target_range = decoded.temperature_target_alarm.temperature_target_range || {};
            decoded.temperature_target_alarm.temperature_target_range.min = readInt16LE(bytes.slice(i + 2, i + 4)) / 10;
            decoded.temperature_target_alarm.temperature_target_range.max = readInt16LE(bytes.slice(i + 4, i + 6)) / 10;
            i += 6;
        }
        // HISTORY DATA
...
function readPlanEvent(type) {
    // 0: not executed, 1: wake, 2: away, 3: home, 4: sleep, 5: occupied, 6: vacant, 7: eco
    switch (type) {
        case 0x00:
            return "not executed";
        case 0x01:
...
function readPlanType(type) {
    // 0: wake, 1: away, 2: home, 3: sleep, 4: occupied, 5: vacant, 6: eco
    switch (type) {
        case 0x00:
            return "wake";
        case 0x01:
...
function readTemperatureCtlStatus(type) {
    // 0: standby, 1: stage-1 heat, 2: stage-2 heat, 3: stage-3 heat, 4: stage-4 heat, 5: em heat,
    // 6: stage-1 cool, 7: stage-2 cool, 8: stage-5 heat
    switch (type) {
        case 0x00:
            return "standby";
        case 0x01:
...
function handle_downlink_response(channel_type, bytes, offset) {
    ...
    switch (channel_type) {
        ...
        case 0x83:
            ...
            d2d_config.enable = readUInt8(bytes[offset + 1]);
            if (d2d_config.enable === 1) {
                d2d_config.d2d_cmd = readD2DCommand(bytes.slice(offset + 2, offset + 4));
                d2d_config.action_type = (readUInt8(bytes[offset + 4]) >>> 4) & 0x0f;
                // action_type=0 action=(0: system off, 1: system on)
                // action_type=1 action=(0: plan(wake), 1: plan(away), 2: plan(home), 3: plan(sleep), 4: plan(occupied), 5: plan(vacant), 6: plan(eco))
                d2d_config.action = readUInt8(bytes[offset + 4]) & 0x0f;
            }
            offset += 5;
        ...
        case 0x96:
            var d2d_config = {};
            d2d_config.plan_type = readUInt8(bytes[offset]);
            d2d_config.enable = readUInt8(bytes[offset + 1]);
            if (d2d_config.enable === 1) {
                d2d_config.uplink_enable = readUInt8(bytes[offset + 2]);
                d2d_config.d2d_cmd = readD2DCommand(bytes.slice(offset + 3, offset + 5));
                d2d_config.time_enable = readUInt8(bytes[offset + 7]);
                if (d2d_config.time_enable === 1) {
                    d2d_config.time = readUInt16LE(bytes.slice(offset + 5, offset + 7));
                }
            }
            offset += 8;

            decoded.d2d_master_config = decoded.d2d_master_config || [];
            decoded.d2d_master_config.push(d2d_config);
            break;
        case 0x4a: // sync_time
            decoded.sync_time = 1;
            offset += 1;
        ...
        case 0xc1:
            ...
            break;
        case 0xc2:
            decoded.plan_type = readUInt8(bytes[offset]);
            offset += 1;
            break;
        case 0xc4:
            decoded.outside_temperature_control_config = decoded.outside_temperature_control_config || {};
            decoded.outside_temperature_control_config.source = readUInt8(bytes[offset]);
            if (decoded.outside_temperature_control_config.source !== 0) {
        ...
        case 0xc8:
            var plan_config = {};
            plan_config.type = readUInt8(bytes[offset]);
            plan_config.temperature_control_mode = readUInt8(bytes[offset + 1]);
            plan_config.fan_mode = readUInt8(bytes[offset + 2]);
            var t = readInt8(bytes[offset + 3]);
            plan_config.temperature_target = t & 0x7f;
            plan_config.temperature_unit = (t >>> 7) & 0x01;
            plan_config.temperature_error = readInt8(bytes[offset + 4]) / 10;
        ...
        case 0xc9:
            var plan_schedule = {};
            plan_schedule.type = bytes[offset];
            plan_schedule.id = bytes[offset + 1] + 1;
            plan_schedule.enable = bytes[offset + 2];

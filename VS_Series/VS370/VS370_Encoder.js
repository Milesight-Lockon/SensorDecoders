...
function milesightDeviceEncode(payload) {
    ...
    if ("pir_illuminance_threshold" in payload) {
        encoded = encoded.concat(setPirIlluminanceThreshold(payload.pir_illuminance_threshold.enable, payload.pir_illuminance_threshold.upper_limit, payload.pir_illuminance_threshold.lower_limit));
    }
    ...
    if ("hibernate_config" in payload) {
        for (var i = 0; i < payload.hibernate_config.length; i++) {
            ...
            encoded = encoded.concat(setHibernateConfig(config.id, config.enable, config.start_time, config.end_time));
        }
    }
...
/**
 * set time zone
 * @param {number} time_zone unit: minute (UTC+8:00 is 480, UTC-3:30 is -210)
 * @example { "time_zone": 480 }
 * @example { "time_zone": -210 }
 */
 ...
 */
function setPirIlluminanceThreshold(enable, upper_limit, lower_limit) {
 ...
 * @param {number} id values: (1: group 1, 2: group 2)
 * @param {number} enable values: (0: disable, 1: enable)
